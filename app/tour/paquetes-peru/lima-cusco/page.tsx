...
export default function LimaCuscoPage() {
  const { language } = useLanguage();
  
  // Filtrar tours de Lima + Cusco
  const limaCuscoTours = tours.filter(tour => 
    tour.id.includes('experiencia-lima-cusco')
  );
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Lima + Cusco' : 'Lima + Cusco'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'Descubre lo mejor de la costa y los Andes en un solo viaje'
            : 'Discover the best of the coast and the Andes in one trip'}
        </p>

        {limaCuscoTours.length === 0 ? (
