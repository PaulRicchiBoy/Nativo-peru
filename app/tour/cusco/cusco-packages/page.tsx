...
export default function CuscoPackagesPage() {
  const { language } = useLanguage();
  
  // Filtrar paquetes de Cusco
  const cuscoPackages = tours.filter(tour => 
    tour.id.includes('cuzco-inolvidable') || 
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Paquetes Cusco' : 'Cusco Packages'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'Paquetes completos para descubrir la capital del Imperio Inca'
            : 'Complete packages to discover the capital of the Inca Empire'}
        </p>

        {cuscoPackages.length === 0 ? (
