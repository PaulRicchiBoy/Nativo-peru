...
import { Map, Clock, DollarSign } from 'lucide-react';

export default function ClassicToursPage() {
  const { language } = useLanguage();
  
  // Filtrar tours clásicos
  const classicTours = tours.filter(tour => 
    tour.id.includes('city-tour') || 
    tour.id.includes('sacred-valley') ||
    tour.id.includes('rainbow-mountain') ||
    tour.id.includes('cusco-classic')
  );
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Tours Clásicos' : 'Classic Tours'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'Los tours imperdibles para conocer la esencia de Cusco'
            : 'Must-do tours to discover the essence of Cusco'}
        </p>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        {tour.duration === 0.5 
                          ? (language === 'es' ? 'Medio día' : 'Half day')
