...
import { Palmtree, Clock, DollarSign } from 'lucide-react';

export default function LimaPage() {
  const { language } = useLanguage();
  
  // Filtrar tours de Lima
  const limaLimaTours = tours.filter(tour => 
    tour.id.includes('lima') || 
    tour.id.includes('city-tour-lima') ||
    tour.id.includes('paracas')
  );
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Tours en Lima' : 'Lima Tours'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'Descubre la Ciudad de los Reyes, su historia, cultura y gastronomía'
            : 'Discover the City of Kings, its history, culture and gastronomy'}
        </p>

        {limaLimaTours.length === 0 ? (
