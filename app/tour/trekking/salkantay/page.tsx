...
import { Mountain, Clock, DollarSign } from 'lucide-react';

export default function SalkantayPage() {
  const { language } = useLanguage();
  
  // Filtrar tours del Salkantay
  const salkantayTours = tours.filter(tour => 
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Salkantay Trek' : 'Salkantay Trek'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'Una de las 25 mejores caminatas del mundo, rodeando el nevado Salkantay'
            : 'One of the 25 best treks in the world, circling the Salkantay snow-capped mountain'}
        </p>

        {salkantayTours.length === 0 ? (
