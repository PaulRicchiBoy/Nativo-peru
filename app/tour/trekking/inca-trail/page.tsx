...
import { TentTree, Clock, DollarSign } from 'lucide-react';

export default function IncaTrailPage() {
  const { language } = useLanguage();
  
  // Filtrar tours del Camino Inca
  const incaTrailTours = tours.filter(tour => 
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-black">
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-4">
          <TentTree className="w-10 h-10 text-emerald-600" />
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">
            {language === 'es' ? 'Camino Inca' : 'Inca Trail'}
          </h1>
        </div>
        <p className="text-xl text-gray-600 dark:text-gray-400 mb-12">
          {language === 'es' 
            ? 'La caminata más famosa de Sudamérica hacia Machu Picchu'
            : 'The most famous trek in South America to Machu Picchu'}
        </p>

        {incaTrailTours.length === 0 ? (
